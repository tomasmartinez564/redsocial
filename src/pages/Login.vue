...
async function onSubmit() {
  ...
  try {
    ...
    router.push('/chat') // te llevamos al feed (GlobalChat) al loguearte
  } catch (e) {
...
<template>
  <section class="container-page grid gap-4 max-w-md">
    ...
    <form @submit.prevent="onSubmit" class="card grid gap-3" aria-labelledby="login-title">
      ...
      <div class="flex items-center gap-3">
        <button class="btn-primary" :disabled="loading">{{ loading ? 'Ingresando…' : 'Entrar' }}</button>
